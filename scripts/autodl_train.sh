...
TRAIN_DATA="data/qa_pairs.json"                   # 训练数据文件名
NUM_EVAL_SAMPLES=50                               # 评测样本数
...
# 复制 qa_pairs.json 为 train_data.json
if [ -f "data/${TRAIN_DATA}" ]; then
    cp "data/${TRAIN_DATA}" data/train_data.json
    DATA_COUNT=$(python3 -c "import json; print(len(json.load(open('data/train_data.json'))))")
    echo "  训练数据: ${DATA_COUNT} 条 (来自 ${TRAIN_DATA})"
elif [ -f "data/train_data.json" ]; then
    DATA_COUNT=$(python3 -c "import json; print(len(json.load(open('data/train_data.json'))))")
    echo "  训练数据: ${DATA_COUNT} 条 (已存在 train_data.json)"
else
    echo "  [WARN] 未找到训练数据，使用 sample_data.json"
...
python3 train/evaluate_model.py \
    ...
    --test-data data/train_data.json \
